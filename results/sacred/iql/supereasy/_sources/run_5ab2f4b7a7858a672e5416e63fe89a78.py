...
def linear_annealed_factor(t_env, start_value, end_value, anneal_start, anneal_end):
    if anneal_start > anneal_end:
        raise ValueError("anneal_start should be less than or equal to anneal_end.")
    if t_env < anneal_start:
        return start_value
    elif t_env >= anneal_end:
        return end_value
    else:
        fraction = (t_env - anneal_start) / (anneal_end - anneal_start)
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size): # 每batch_size个episodes更新一次网络参数
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            for _ in range(n_test_runs):
                ...
                test_performance += episode_info["reward"].sum()/episode_info['reward'].shape[0]            
        ...
        if (runner.t_env - last_log_T) >= args.log_interval: 
            ...
            # logger.print_recent_stats() # 每隔log_interval打印一次到屏幕
