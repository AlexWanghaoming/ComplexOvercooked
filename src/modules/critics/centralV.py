...
class CentralVCritic(nn.Module):
    ...
    def forward(self, batch, t=None):
        inputs, bs, max_t = self._build_inputs(batch, t=t) # input shape: (bs, max_t, n_agents, input_shape)
    ...
    def _build_inputs(self, batch, t=None):
        """
        构造critic的输入时，采用的是centralized critic的方式，即每个agent都能看到所有agent的信息，
        对于overcooked环境，state和obs可以是相同的，但是对于其他环境，state可能时obs的全集
        """
        bs = batch.batch_size
